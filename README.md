...
- 编辑可选择“忠实复刻”保留原文总体意思，或选择分析得到的采编角度，再确认目标读者、语气和目标字数，由 OpenAI-compatible provider 生成新的 `editing` 稿件。
...
当前采集范围是微信公众号与普通网页的公开内容，AI 通过 OpenAI-compatible provider 接入，并已兼容智谱 GLM 的版本化 API 根地址；Nuxt 阅读前台本轮不增加 AI 操作。公众号自动群发、小红书等平台适配器、RSS、OCR、全文搜索、多租户和计费仍未实现。
...
3. 选择“忠实复刻”保留核心主题、事实、观点关系和总体结论，或选择一个采编角度；再填写目标读者、语气和目标字数，生成任务完成后打开新建的 `editing` 稿件。
